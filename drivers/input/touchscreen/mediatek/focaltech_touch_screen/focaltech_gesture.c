...
#define KEY_FTS_SENSOR 251
extern u8 gTGesture;
extern struct input_dev *fts_key_dev;
/*****************************************************************************
...
static int fts_gesture_report(int gesture_id)
{
    int gesture;
...
int fts_gesture_readdata(struct i2c_client *client)
{
    ...
    //if (fts_gesture_fw())
    {
        ...
        buf[0] = FTS_REG_GESTURE_OUTPUT_ADDRESS;
        FTS_DEBUG("[GESTURE]PointNum=%d,gestrue_id=%d\n", pointnum,gestrue_id);
 	...
 	if(gestrue_id == GESTURE_DOUBLECLICK)
                {
                    gTGesture = 'u';
                    input_report_key(fts_key_dev,KEY_FTS_SENSOR, 1);
                    input_report_key(fts_key_dev,KEY_FTS_SENSOR, 0);
                    input_sync(fts_key_dev);
              }
              else
                {
                    gTGesture = fts_gesture_report(gestrue_id);
                    input_report_key(fts_key_dev,KEY_FTS_SENSOR, 1);
                    input_report_key(fts_key_dev,KEY_FTS_SENSOR, 0);
                    input_sync(fts_key_dev);
                }
       // fts_gesture_report(fts_key_dev,gestrue_id);
	...
	FTS_ERROR("geroge [GESTURE]IC  need gesture lib to support gestures. \n");
        return 0;
    }*/
#if 0
