...
#define FTS_CTP_VENDOR_ZGD    (0x17)
#if FTS_GESTURE_EN
struct input_dev *fts_key_dev;
extern int bEnTGesture;
#endif

#if (defined(FTS_GESTURE_EN)||(defined(FTS_PSENSOR_EN)))
...
static int touch_event_handler(void *unused)
{
 ...
 if(g_iLight_sensor){
	if (bEnTGesture) {
        ret = fts_i2c_read_reg(ts_data->client, FTS_REG_GESTURE_EN, &state);
        if (ret<0)
        {
        ...
        if (state == 1)
        {
            fts_gesture_readdata(ts_data->client);
            continue;
        }
        }
       }
...
if(g_iLight_sensor){
	...
	fts_key_dev->name = "TPFTS_GESTURE";
    ...
    ret = fts_gesture_init(tpd->dev,client);
    if (ret) {
...
if (NORMAL_BOOT == get_boot_mode()){
   ...
   FTS_DEBUG("ftm  recovery meta mode don't update tp \n");
...
void fts_ftm_force_update(char * ftm_update)
{
    ...
    if (ret) {
        FTS_ERROR("ftm init fw upgrade fail");
    }
}

int fts_tpd_get_fw_version(void)
{
    u8 tp_ver_info;
...
char *fts_tpd_get_fw_vendor_name(void){

     u8 tp_vendor_id = 0;
     fts_i2c_read_reg(fts_i2c_client, FTS_REG_VENDOR_ID, &tp_vendor_id);
     FTS_DEBUG("[TPD] geroge tp_vendor_id = 0x%x\n", tp_vendor_id);

     sprintf(tpd_venor_name, "%s", fts_get_vendor_name(tp_vendor_id));
...
int get_tp_fw_info(char *buf, void *arg0) 
{
    return sprintf(buf, 
        "%s-%s-%s-%x",
        fts_tpd_get_fw_vendor_name(),
        CONFIG_ARCH_MTK_PROJECT,
        FTS_DRIVER_NAME,
        fts_tpd_get_fw_version());
}
#endif
/*****************************************************************************
*  Name: tpd_remove
