...
extern bool ftm_fts_force_update ; //ftm force update
/*****************************************************************************
* Static variables
*****************************************************************************/
...
static int fts_ctpm_check_fw_ver(struct i2c_client *client)
{
    ...
    FTS_DEBUG("[UPGRADE]: uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x , ftm_fts_force_update = %d \n", uc_tp_fm_ver, uc_host_fm_ver, ftm_fts_force_update);
    if ((uc_tp_fm_ver < uc_host_fm_ver) ||ftm_fts_force_update )
    {
        return 1;
    }
    else
