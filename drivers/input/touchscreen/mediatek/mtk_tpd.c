...
static ssize_t show_tpd_fw_version(struct device_driver *dev,char *buf)
{
...
static ssize_t show_tpd_fw_vendor_info(struct device_driver *dev,char *buf)
{
...
static ssize_t show_tpd_fw_chip_info(struct device_driver *dev,char *buf)
{
    if( g_tpd_drv )
    {
...
static DRIVER_ATTR(tpd_fw_version, S_IWUSR | S_IRUGO, show_tpd_fw_version, NULL);
static DRIVER_ATTR(tpd_fw_vendor_info, S_IWUSR | S_IRUGO, show_tpd_fw_vendor_info, NULL);
static DRIVER_ATTR(tpd_fw_chip_info, S_IWUSR | S_IRUGO, show_tpd_fw_chip_info, NULL);

static struct driver_attribute *mtk_tpd_attr_list[] = {
							&driver_attr_tpd_fw_version,
							&driver_attr_tpd_fw_vendor_info,
							&driver_attr_tpd_fw_chip_info,
};
//tinno add begin
...
//tinno add begin
	ret_tp = mtk_tpd_create_attr(&tpd_driver.driver);
	if (0 != ret_tp) {
