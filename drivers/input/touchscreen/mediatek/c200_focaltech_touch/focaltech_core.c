...
*****************************************************************************/
#define FTS_DRIVER_NAME                     "ft6636u"
...
#define FTS_CTP_VENDOR_YEJI_NEW     (0x11)
...
#define FTS_CTP_VENDOR_HLT          (0x82)
#define FTS_CTP_VENDOR_TCL          (0xe0)
#define FTS_CTP_VENDOR_YIXING       (0x11)
#define FTS_CTP_VENDOR_HLT_R    (0x8A)
#define FTS_CTP_VENDOR_DIJING_R       (0x68)
...
extern void tpd_power_ldo(int onoff);
#endif
extern char tpd_desc[50];
struct input_dev *fts_key_dev;
#if FTS_GESTURE_EN
extern int bEnTGesture;
#endif

bool ftm_fts_force_update = false;
...
static char *fts_get_vendor_name(u8 vendor_id)
{
	...
	case FTS_CTP_VENDOR_TCL:
            return "TCL";
            break;
	case FTS_CTP_VENDOR_HLT:
	     return "HLT";
	case FTS_CTP_VENDOR_HLT_R:
	     return "HLT";
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  fts_key_dev= input_allocate_device();
if (! fts_key_dev) 
{
	FTS_ERROR("[fts]SY_key_dev: fail!\n");
}
   __set_bit(EV_KEY,  fts_key_dev->evbit);
   __set_bit(251,	fts_key_dev->keybit);
__set_bit(KEY_POWER,  fts_key_dev->keybit);
fts_key_dev->id.bustype = BUS_HOST;
fts_key_dev->name = "TPFT5XX_GESTURE";
if(input_register_device(fts_key_dev))
{
	FTS_ERROR("[fts]fts_key_dev register : fail!\n");
}else
{
	FTS_ERROR("[fts]fts_key_dev register : success!!\n");
} 
#endif
   ...
   if (NORMAL_BOOT == get_boot_mode()){
		...
		FTS_DEBUG("[TPD] not upgrade in recovery and ftm mode !\n");
	}


#endif

     fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &tp_vendor_id);
     FTS_DEBUG("[TPD] geroge tp_vendor_id = 0x%x\n", tp_vendor_id);
     sprintf(tpd_desc, "%s", fts_get_vendor_name(tp_vendor_id));
...
void fts_ftm_force_update(char * ftm_update)
{
    FTS_DEBUG("start fts_ftm_force_update\n");
    ftm_fts_force_update = true;
#if FTS_AUTO_UPGRADE_EN
    fts_ctpm_upgrade_init();
#endif
...
void fts_tpd_get_fw_version( char * fw_tp_ver )
{
    u8 tp_ver_info;
    fts_i2c_read_reg(fts_i2c_client, FTS_REG_FW_VER, &tp_ver_info);
    FTS_DEBUG("[TPD] geroge tp_ver_info = 0x %x\n", tp_ver_info);

    sprintf(fw_tp_ver, "%x", tp_ver_info);
}

int fts_tpd_get_fw_version_ext(void)
{
    u8 tp_ver_info;
    fts_i2c_read_reg(fts_i2c_client, FTS_REG_FW_VER, &tp_ver_info);
    FTS_DEBUG("[TPD] geroge tp_ver_info = 0x %x\n", tp_ver_info);
...
void fts_tpd_get_fw_vendor_name(char * fw_vendor_name)
{
...
static struct tpd_driver_t tpd_device_driver =
{
    ...
    .suspend = tpd_suspend,
    .resume = tpd_resume,
    .tpd_get_fw_version = fts_tpd_get_fw_version,
    .tpd_get_fw_vendor_name = fts_tpd_get_fw_vendor_name,
    .tpd_ftm_force_update = fts_ftm_force_update,
    .attrs = {
        .attr = fts_attrs,
