...
extern	int target_speed;   //target speed set by app layer.
extern	int speed_current_map[10];   //actual current/ma for speed level.
//
...
/*scc end*/
	...
	{
		if (info->sw_jeita.sm == TEMP_T3_TO_T4)//temperature between 45~60
		{
		...
		else if (info->sw_jeita.sm == TEMP_T2_TO_T3)//temperature between 10~45
		{
		...
		else if (info->sw_jeita.sm == TEMP_T1_TO_T2)//temperature between 0~10
		{
...
/*scc begin add by sunhonghui*/
		current_limit = pdata->charging_current_limit;
        temp_CC_value = speed_current_map[target_speed];

	    printk("geroge temp_CC_value = %d\n",temp_CC_value);

	    if(temp_CC_value != 0)
	        pdata->charging_current_limit = pdata->charging_current_limit > temp_CC_value ?  temp_CC_value : pdata->charging_current_limit;
	    else
	        pdata->charging_current_limit = current_limit;

	    printk("geroge pdata->charging_current_limi = %d, temp_CC_value = %d\n",pdata->charging_current_limit,temp_CC_value);
...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)) {
			...
			#ifdef TINNO_BUILD_GCF
				chr_err("[charger] TINNO_BUILD_GCF,STANDARD_HOST charging current = 0 \n");			
				pdata->input_current_limit = 0;
				pdata->charging_current_limit = 0;	/* it can be larger */
			#else
		...
		#ifdef TINNO_BUILD_GCF
			chr_err("[charger] TINNO_BUILD_GCF,CHARGING_HOST charging current = 0\n");
			pdata->input_current_limit = 0;
			pdata->charging_current_limit = 0;		
		#else
