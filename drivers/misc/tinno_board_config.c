...
static int __init tinno_board_config(void)
{
   char *p = NULL;
   int i;
   int j = 0;
   ...
   {
      printk(KERN_ERR "saved_command_line is NULL exist\n");
      return -1;
   }
   ...
   if (p == NULL)
   {
      printk(KERN_ERR "can not find androidboot.Market_Area in saved_command_line");
      return -1;
   }

   market_area_len = strlen(Market_AREA);

   printk(KERN_ERR "market_area_len = %d", market_area_len); 
   ...
   if (p == NULL)
   {
      printk(KERN_ERR "can not find androidboot.Sensor_Info in saved_command_line");
      return -1;
   }
   ...
   if (p != NULL)
   {
       ...
       printk("can not find androidboot.Audio_Pa_Mode in saved_command_line");
   }
   ...
   {
      printk(KERN_ERR "failed to get market area value");
   }

   printk(KERN_ERR "HJDDbgSensor, Market_Area=%s, sensor_info_buffer=%s \n", Market_Area, sensor_info_buffer);

   // For sensor custom testing
   // memset(sensor_info_buffer, 0, sizeof(sensor_info_buffer));
   // strcpy(sensor_info_buffer, "OAPLMH");
   // printk(KERN_ERR "HJDDbgSensor2, Market_Area=%s, sensor_info_buffer=%s \n", Market_Area, sensor_info_buffer);
...
early_initcall(tinno_board_config);
