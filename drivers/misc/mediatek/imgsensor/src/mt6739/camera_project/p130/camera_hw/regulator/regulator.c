	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	if (pregulator) {
	   ...
	   /*add for VCAMD 1.1v and 1.2V compatible by liaoshuang 2018-2-9*/
	   #ifdef CONFIG_MTK_VCAMD_COMPATIBLE
			if(IMGSENSOR_HW_PIN_DVDD == pin && ((sensor_idx == IMGSENSOR_SENSOR_IDX_SUB) || (strcmp(get_current_sensor_name(),"s5k4h8_sunwin_p130_mipi_raw") == 0))){
				pmic_config_interface(PMIC_RG_VCAMD_VOCAL_ADDR,0x0a,0xf,0);
			}
       #endif
			...
			/*add for VCAMD 1.1v and 1.2V compatible by liaoshuang 2018-1-18*/
#ifdef CONFIG_MTK_VCAMD_COMPATIBLE
			if(IMGSENSOR_HW_PIN_DVDD == pin && (sensor_idx == IMGSENSOR_SENSOR_IDX_SUB || (strcmp(get_current_sensor_name(),"s5k4h8_sunwin_p130_mipi_raw") == 0))){
