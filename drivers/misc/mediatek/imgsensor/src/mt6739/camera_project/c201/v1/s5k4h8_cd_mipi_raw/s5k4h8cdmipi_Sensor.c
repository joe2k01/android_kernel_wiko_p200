...
#define PFX "S5K4H8CD_camera_sensor"
//#define LOG_1 LOG_INF("S5K4H8CD,MIPI 4LANE\n")
...
#define S5K4H8CD_OTP_USE
#ifdef S5K4H8CD_OTP_USE
extern bool s5k4h8cd_otp_daling_update_wb(void);
...
static void write_cmos_sensor_8(kal_uint16 addr, kal_uint8 para)
{
 ...
 void S5K4H8CD_wordwrite_cmos_sensor(u16 addr, kal_uint16 para)
{
	char puSendCmd[4] = {(char)(addr >> 8) , (char)(addr & 0xFF) ,  (char)(para >> 8),	(char)(para & 0xFF) };
	iWriteRegI2C(puSendCmd , 4,imgsensor.i2c_write_id);
}

 void S5K4H8CD_bytewrite_cmos_sensor(u16 addr, kal_uint16 para)
{
	char puSendCmd[4] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF)};
	iWriteRegI2C(puSendCmd , 3,imgsensor.i2c_write_id);
}

kal_uint16 S5K4H8CD_byteread_cmos_sensor(u16 addr)
{
...
*
*************************************************************************/
static kal_uint8 s5k4h8cd_get_lens_id(void) 
{
    kal_uint8 flag = 0,i=0;
    kal_uint16 addr = 0x0A06,lens_id=0xffff;
    write_cmos_sensor(0x0100,0x0100);
    mdelay(15);
    S5K4H8CD_bytewrite_cmos_sensor(0x0A02,0x0f); 
    S5K4H8CD_bytewrite_cmos_sensor(0x0A00, 0x01); 
    mdelay(15);
    flag = S5K4H8CD_byteread_cmos_sensor(0x0A04);
    LOG_INF("s5k4h8cd Read flag = 0x%x\n", flag);
    if(flag!=0x01)
    	addr=0x0A1C;
    lens_id=S5K4H8CD_byteread_cmos_sensor(addr);
    LOG_INF("s5k4h8cd Read lens id = 0x%x\n", lens_id);
    S5K4H8CD_bytewrite_cmos_sensor(0x0A00, 0x00); 
    write_cmos_sensor(0x0100,0x0000);
    if(lens_id==0x68)
	return 1;
    else
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12)+1;
            ...
            if ((*sensor_id == imgsensor_info.sensor_id)&&(!s5k4h8cd_get_lens_id())) {               
                LOG_INF("s5k4h8cd i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
                return ERROR_NONE;
            }   
            LOG_INF("s5k4h8cd Read sensor id fail, write id: 0x%x, sensor id = 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
	...
	//sensor have two i2c address 0x5a 0x5b & 0x21 0x20, we should detect the module used i2c address
        ...
        do {
            ...
            if (sensor_id == imgsensor_info.sensor_id){                
			 ...
			 LOG_INF("s5k4h8cd Read sensor id fail i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 S5K4H8_CD_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
