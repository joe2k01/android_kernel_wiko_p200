...
extern bool s5k4h8_otp_daling_update_wb(void);
...
*
*************************************************************************/
//TINNO zhihua.lu CCAAGO-842 del the stream on function when camera setting.
#if 0
...
*
*************************************************************************/
static kal_uint8 s5k4h8_get_lens_id(void) 
{
    ...
    mdelay(15);
    flag = S5K4H8_byteread_cmos_sensor(0x0A04);
    LOG_INF("s5k4h8 flag = 0x%x\n", flag);
    if(flag!=0x01)
    	addr=0x0A1C;
    lens_id=S5K4H8_byteread_cmos_sensor(addr);
	LOG_INF("s5k4h8 Read lens id = 0x%x\n", lens_id);
 	S5K4H8_bytewrite_cmos_sensor(0x0A00, 0x00); 
    write_cmos_sensor(0x0100,0x0000);
    if(lens_id==0x68)
	return 1;
    else
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
            ...
            if ((*sensor_id == imgsensor_info.sensor_id)&&(s5k4h8_get_lens_id())) {               
                LOG_INF("s5k4h8 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
                return ERROR_NONE;
            }   
            LOG_INF("s5k4h8 Read sensor id fail, write id: 0x%x, sensor id = 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
	...
	//sensor have two i2c address 0x5a 0x5b & 0x21 0x20, we should detect the module used i2c address
        ...
        do {
            ...
            if (sensor_id == imgsensor_info.sensor_id) {                
			 ...
			 LOG_INF("s5k4h8 Read sensor id fail i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
