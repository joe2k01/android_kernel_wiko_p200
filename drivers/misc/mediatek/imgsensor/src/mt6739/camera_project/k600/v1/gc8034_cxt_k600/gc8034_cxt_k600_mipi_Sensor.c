...
#include "gc8034_cxt_k600_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC8034_CXT_camera_sensor"
#define LOG_1 LOG_INF("GC8034_CXT,MIPI 4LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static	void gc8034_read_otp_group(kal_uint8 page, kal_uint8 addr, kal_uint8 *buff, int size);
static  int  gamma_index;
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC8034_CXT_K600_SENSOR_ID,       /*record sensor id defined in Kd_imgsensor.h*/
	...
	.mipi_settle_delay_mode = MIPI_SETTLEDELAY_MANUAL,//MIPI_SETTLEDELAY_AUTO,
...
typedef struct gc8034_cxt_k600_otp_info
{
	kal_uint16 dd_cnt;
	kal_uint16 dd_flag;
#ifdef GC8034_OTP_FOR_CUSTOMER
	...
	kal_uint16 af_macro;// 
#endif
	kal_uint16 reg_page[10];//ackman change from 5
	kal_uint16 reg_addr[10];	
	kal_uint16 reg_value[10];	
	kal_uint16 reg_flag;
	kal_uint16 reg_num;
}gc8034_cxt_k600_otp_info;

static gc8034_cxt_k600_otp_info gc8034_otp_info;
#ifdef GC8034_OTP_FOR_CUSTOMER
u16 gc8034_cxt_k600_af_data[2];
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x04:
			...
			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				...
				gc8034_cxt_k600_af_data[0] = gc8034_otp_info.af_infinity;
                		gc8034_cxt_k600_af_data[1] = gc8034_otp_info.af_macro;
			} else
...
static void gc8034_gcore_update_wb(void)
{
	...
	if (0x01 == (gc8034_otp_info.wb_flag & 0x01)) {
		...
		LOG_INF("GC8034_CXT_K600__OTP_UPDATE_AWB:r_gain_current = 0x%x , b_gain_current = 0x%x , base_gain = 0x%x \n",r_gain_current,b_gain_current,base_gain);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				LOG_INF("gc8034_otp_info.module_id: 0x%x\n", gc8034_otp_info.module_id);
				if(gc8034_otp_info.module_id == 0x13){
...
UINT32 GC8034_CXT_K600_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
