...
#include "ov8856_sunwin_p200_mipi_raw.h"
...
static int BG_Ratio_Typical= 0x175 ; 

int ov8856_read_otp_sub(void)
{
	...
	if(addr != 0)
	{
		otp_info_sub.flag = 0xC0; // valid info and AWB in OTP
		otp_info_sub.module_integrator_id = ov8856_read_i2c(addr);
		otp_info_sub.lens_id = ov8856_read_i2c( addr + 1); 
		otp_info_sub.production_year = ov8856_read_i2c( addr + 2); 
		otp_info_sub.production_month = ov8856_read_i2c( addr + 3); 
		otp_info_sub.production_day = ov8856_read_i2c(addr + 4); 
		temp = ov8856_read_i2c(addr + 7);
		otp_info_sub.rg_ratio = (ov8856_read_i2c(addr + 5)<<2) + ((temp>>6) & 0x03);
		otp_info_sub.bg_ratio = (ov8856_read_i2c(addr + 6)<<2) + ((temp>>4) & 0x03);
	}
	else
	{
		otp_info_sub.flag = 0x00; // not info and AWB in OTP
		otp_info_sub.module_integrator_id = 0;
		otp_info_sub.lens_id = 0;
		otp_info_sub.production_year = 0;
		otp_info_sub.production_month = 0;
		otp_info_sub.production_day = 0;
		otp_info_sub.rg_ratio = 0;
		otp_info_sub.bg_ratio = 0;
	}
	...
	if(addr != 0)
	{
		for(i=0;i<240;i++)
		{
			otp_info_sub.lenc[i]=ov8856_read_i2c(addr + i);
			checksum2 += otp_info_sub.lenc[i];
		}
		checksum2 = (checksum2)%255 +1;
		otp_info_sub.checksum = ov8856_read_i2c((addr + 240));

		if(otp_info_sub.checksum == checksum2)
		{
			otp_info_sub.flag |= 0x10;
		}
	}
	else
	{
	...
	return otp_info_sub.flag;
}

int ov8856_apply_otp_sub(void)
{
	int rg, bg, R_gain, G_gain, B_gain, Base_gain, temp, i;
	// apply OTP WB Calibration
	if (otp_info_sub.flag & 0x40)
	{
		rg = otp_info_sub.rg_ratio;
		bg = otp_info_sub.bg_ratio;
		//calculate G gain
	...
	if (otp_info_sub.flag & 0x10)
	{
		...
		for(i=0;i<240;i++)
		{
			ov8856_write_i2c(0x5900 + i, otp_info_sub.lenc[i]);
		}
