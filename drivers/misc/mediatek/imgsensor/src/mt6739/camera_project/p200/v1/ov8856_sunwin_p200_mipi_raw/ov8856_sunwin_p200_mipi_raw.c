...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8856_SUNWIN_P200_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
#ifdef OV8856_OTP_Enable
    extern void ov8856_read_otp_sub(void);
    extern void ov8856_apply_otp_sub(void);
...
static void vga_setting_120fps(void)
{
		...
		write_cmos_sensor(0x3812, 0x00);
		write_cmos_sensor(0x3813, 0x02);//03
		write_cmos_sensor(0x3814, 0x03);//07
		write_cmos_sensor(0x3815, 0x01);
		write_cmos_sensor(0x3820, 0x90);
		write_cmos_sensor(0x3821, 0x67);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            LOG_INF("douyang-sensor-id = %x" , *sensor_id);
		    LOG_INF("douyang-imgsensor-id = %x" , imgsensor_info.sensor_id);	
			//begin add by douyang 
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    sensor_info->SensorHorFOV = 71;
    sensor_info->SensorVerFOV = 56;
	switch (scenario_id) {
...
UINT32 OV8856_SUNWIN_P200_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
