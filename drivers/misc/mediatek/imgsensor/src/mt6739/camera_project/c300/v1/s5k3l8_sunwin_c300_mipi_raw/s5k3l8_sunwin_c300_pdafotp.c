...
#include "s5k3l8_sunwin_c300_rear_mipi_raw.h"
...
#define CAM_CAL_DRVNAME "S5K3L8_SUNWIN_C300_CAL_DRV"
...
kal_uint8 S5K3L8_SUNWIN_C300_CheckID[]= {0x10,0xff,0x00,0x40,0x88};
...
typedef struct s5k3l8_sunwin_c300_otp_info
{
	kal_uint8 info_awb_flag;
	kal_uint8 module_id;
	kal_uint8 lens_id;
	kal_uint8 year;
	kal_uint8 month;
	...
	kal_uint16 golden_gg;
	kal_uint16 awb_unit_data[4];
	kal_uint16 awb_golden_data[4];
	kal_uint8 af_flag;
	kal_uint16 af_infinity;	
	kal_uint16 af_macro;
	kal_uint8 lsc_flag;
	kal_uint8 lsc_data[lsc_size];
	kal_uint8 pdaf_flag;
	kal_uint8 pdaf_data[pdaf_size];
}s5k3l8_sunwin_c300_otp_info;

static s5k3l8_sunwin_c300_otp_info  s5k3l8_sunwin_otp_info;

bool s5k3l8_sunwin_c300_read_eeprom_data(void)
{
	...
	if(s5k3l8_sunwin_otp_info.info_awb_flag == 0x01){
		...
		CAM_CALDB("s5k3l8_sunwin_c300_read_eeprom_data info_awb is empty or invalid\n");
		//return false;
	}
	...
	printk("s5k3l8_sunwin_otp_info rg_gain = 0x%x,bg_gain = 0x%x,gg_gain = 0x%x\n",
							s5k3l8_sunwin_otp_info.rg_gain,s5k3l8_sunwin_otp_info.bg_gain,s5k3l8_sunwin_otp_info.gg_gain);
	printk("s5k3l8_sunwin_otp_info golden_rg = 0x%x,golden_bg = 0x%x,golden_gg = 0x%x\n",
	...
	if(read_cmos_sensor_byte(0x078D) == check_sum % 256){
		printk("s5k3l8_sunwin_otp_info lsc data is suceess\n");
	}
	//pdaf data
	check_sum = 0;
	j = 0;
	...
	if(s5k3l8_sunwin_otp_info.pdaf_flag == 0x01){
		...
		CAM_CALDB("s5k3l8_sunwin_c300_read_eeprom_data pdaf data is empty or invalid\n");
		
	}
	#if 0
	for(i = 0; i++; i < pdaf_size){
		CAM_CALDB("s5k3l8_sunwin_c300_read_eeprom_data s5k3l8_sunwin_otp_info.pdaf_data[%d] = %d\n",i,s5k3l8_sunwin_otp_info.pdaf_data[i]);
	}
	CAM_CALDB("s5k3l8_sunwin_c300_read_eeprom_data 0x0D0D = %d\n",read_cmos_sensor_byte(0x0D0D));
	#endif
	if(read_cmos_sensor_byte(0x0D0D) == check_sum % 256){
		printk("s5k3l8_sunwin_c300_read_eeprom_data pdaf data is suceess\n");
	}
...
bool s5k3l8_sunwin_c300_otp_wb_update(void)
{
	kal_uint16 R_GAIN;
	kal_uint16 B_GAIN;
	kal_uint16 G_GAIN;
	...
	CAM_CALDB("s5k3l8_sunwin_c300_otp_wb_update start\n");
	CAM_CALDB("s5k3l8_sunwin_c300_otp_wb R.Gr_Ratio = 0x%x,s5k3l8_sunwin_otp_info.bg_gain =0x%x",s5k3l8_sunwin_otp_info.rg_gain,s5k3l8_sunwin_otp_info.bg_gain);

	if(!s5k3l8_sunwin_otp_info.rg_gain || !s5k3l8_sunwin_otp_info.bg_gain||!BG_Ratio_typical ||!RG_Ratio_typical)
	{
		CAM_CALDB("s5k3l8_sunwin_c300_otp_wb: OTP WB ratio Data Err!\n");
		return false;
	}
	...
	CAM_CALDB("s5k3l8_sunwin_c300_otp_wb:[R_GAIN=0x%x],[G_GAIN=0x%x],[B_GAIN=0x%x] \n",R_GAIN, G_GAIN, B_GAIN);
	...
	CAM_CALDB("s5k3l8_sunwin_c300_otp_wb_update end\n");    
	return true;;
}
#endif
/********************************************************************************/
		...
		case CAM_CALIOC_G_READ:
			...
			{	
				if(ptempbuf->u4Offset == 0x07){
				 ...
				 memcpy(pWorkingBuff,&S5K3L8_SUNWIN_C300_CheckID[1],ptempbuf->u4Length);
			}
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	CAM_CAL_class = class_create(THIS_MODULE, "S5K3L8_SUNWIN_C300_CAL_DRV");
...
static int __init S5K3L8_SUNWIN_C300_CAM_CAL_init(void)
{
...
static void __exit S5K3L8_SUNWIN_C300_CAM_CAL_exit(void)
{
	platform_driver_unregister(&g_stCAM_CAL_Driver);
}

module_init(S5K3L8_SUNWIN_C300_CAM_CAL_init);
module_exit(S5K3L8_SUNWIN_C300_CAM_CAL_exit);

MODULE_DESCRIPTION("S5K3L8_SUNWIN_C300_CAL driver");
MODULE_AUTHOR("tinno");
MODULE_LICENSE("GPL");
