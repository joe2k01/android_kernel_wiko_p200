...
#include "imx135_sunwin_c300_mipi_raw.h"
...
#define golden_r_g 617 //628
#define golden_b_g 700 //666
...
#define NEW_UNLOCK_IOCTL
...
#endif
	{
		...
		else
		{
			...
			if(NULL == pBuff)
			{
				LOG_INF("[IMX135_CAM_CAL] ioctl allocate mem failed\n");
				return -ENOMEM;
			}
			...
			if(_IOC_WRITE & _IOC_DIR(a_u4Command))
			{
				...
				{	 //get input structure address
					kfree(pBuff);
					LOG_INF("[IMX135_CAM_CAL] ioctl copy from user failed\n");
					return -EFAULT;
				}
			}
		}
		...
		//fix warning MSG	  LOG_INF("[IMX135_CAM_CAL] init Working buffer address 0x%x  command is 0x%08x\n", pWorkingBuff, a_u4Command);
		...
		switch(a_u4Command)
		{
			...
			case CAM_CALIOC_G_READ:
				LOG_INF("[IMX135_CAM_CAL] Read CMD \n");
			#ifdef CAM_CALGETDLT_DEBUG            
				do_gettimeofday(&ktv1);
			#endif 
				LOG_INF("[IMX135_CAM_CAL] offset %d \n", ptempbuf->u4Offset);
				LOG_INF("[IMX135_CAM_CAL] length %d \n", ptempbuf->u4Length);
				// LOG_INF("[IMX135_CAM_CAL] Before read Working buffer address 0x%x \n", pWorkingBuff);
				...
				// LOG_INF("[IMX135_CAM_CAL] After read Working buffer address 0x%x \n", pWorkingBuff);
		...
		if(_IOC_READ & _IOC_DIR(a_u4Command))
		{
			...
			LOG_INF("[IMX135_CAM_CAL] to user length %d \n", ptempbuf->u4Length);
			...
			if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
			{
				kfree(pBuff);
				kfree(pWorkingBuff);
				LOG_INF("[IMX135_CAM_CAL] ioctl copy to user failed\n");
				return -EFAULT;
			}
		}
	...
	{
		LOG_INF("[IMX135 CAM_CAL] CAM_CAL_Open\n");
		...
		//{
		//	  LOG_INF("[IMX135_CAM_CAL] Fail to enable analog gain\n");
		//	  return -EIO;
		//}
		//#endif	
...
#define CAM_CAL_DYNAMIC_ALLOCATE_DEVNO 1
	inline static int RegisterCAM_CALCharDrv(void)
	{
		...
		{
			LOG_INF("[IMX135_CAM_CAL] Allocate device no failed\n");
		...
		{
			LOG_INF("[IMX135_CAM_CAL] Register device no failed\n");
		...
		if(NULL == g_pCAM_CAL_CharDrv)
		{
			...
			LOG_INF("[IMX135_CAM_CAL] Allocate mem for kobject failed\n");
		...
		{
			LOG_INF("[IMX135_CAM_CAL] Attatch file operation failed\n");
		...
		CAM_CAL_class = class_create(THIS_MODULE, "IMX135_CAM_CALdrv");
	...
	static int __init IMX135_SUNWIN_C300_CAM_CAL_i2C_init(void)
	{
		...
		int i4RetValue = 0;
		LOG_INF("[IMX135_CAM_CAL]\n");
		//Register char driver
		i4RetValue = RegisterCAM_CALCharDrv();
		if(i4RetValue){
			LOG_INF(" [IMX135_CAM_CAL] register char device failed!\n");
			return i4RetValue;
		}
		LOG_INF(" [IMX135_CAM_CAL] Attached!! \n");
		...
		LOG_INF(" IMX135_CAM_CAL  Attached Pass !! \n");
		return 0;
	}
	
	static void __exit IMX135_SUNWIN_C300_CAM_CAL_i2C_exit(void)
	{
		platform_driver_unregister(&g_stCAM_CAL_Driver);
	}
	
module_init(IMX135_SUNWIN_C300_CAM_CAL_i2C_init);
module_exit(IMX135_SUNWIN_C300_CAM_CAL_i2C_exit);
