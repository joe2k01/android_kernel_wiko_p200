...
#include "s5k3p3st_sunny_c300_front_mipi_raw.h"

#define PFX "S5K3P3ST_SUNNY_C300_FRONT_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3P3ST_SUNNY_C300_FRONT_SENSOR_ID,
...
uint16_t RGr_ratio = 0x0000;
uint16_t BGr_ratio = 0x0000;
uint16_t GbGr_ratio = 0x0000;

uint16_t RGr_golden_ratio = 0x0000;
uint16_t BGr_golden_ratio = 0x0000;
uint16_t GbGr_golden_ratio = 0x0000;

#define RGr_ratio_Typical  420//617  //modify by v3702 
#define BGr_ratio_Typical  644//577  //modify by v3702
...
kal_uint8 s5k3p3st_sunny_c300_lsc_data[1868];

typedef struct s5k3p3st_sunny_c300_otp_info
{
	...
	uint16_t golden_gg;
	uint16_t lsc_flag;
}s5k3p3st_sunny_c300_otp_info;

static s5k3p3st_sunny_c300_otp_info  s5k3p3st_sunny_otp_info;

bool s5k3p3st_sunny_c300_read_eeprom_data(void)
{

	s5k3p3st_sunny_otp_info.info_awb_flag = read_cmos_sensor_byte(0x0019);
	if(0x01 == s5k3p3st_sunny_otp_info.info_awb_flag ){
		//module id
		s5k3p3st_sunny_otp_info.module_id = read_cmos_sensor_byte(0x0001);
		//year month day
		s5k3p3st_sunny_otp_info.year = read_cmos_sensor_byte(0x0005);
		s5k3p3st_sunny_otp_info.month = read_cmos_sensor_byte(0x0006);
		s5k3p3st_sunny_otp_info.day = read_cmos_sensor_byte(0x0007);
		//lens id
		s5k3p3st_sunny_otp_info.lens_id = read_cmos_sensor_byte(0x0002);
		//awb
		RGr_ratio = s5k3p3st_sunny_otp_info.rg_gain = ((read_cmos_sensor_byte(0x0001A) << 8) & 0xff00) | (read_cmos_sensor_byte(0x001B) & 0x00ff);
		BGr_ratio = s5k3p3st_sunny_otp_info.bg_gain = ((read_cmos_sensor_byte(0x001C) << 8) & 0xff00) | (read_cmos_sensor_byte(0x001D) & 0x00ff);
		GbGr_ratio = s5k3p3st_sunny_otp_info.gg_gain = ((read_cmos_sensor_byte(0x001E) << 8) & 0xff00) | (read_cmos_sensor_byte(0x001F) & 0x00ff);

		RGr_golden_ratio = s5k3p3st_sunny_otp_info.golden_rg = ((read_cmos_sensor_byte(0x0020) << 8) & 0xff00) | (read_cmos_sensor_byte(0x0021) & 0x00ff);
		BGr_golden_ratio = s5k3p3st_sunny_otp_info.golden_bg = ((read_cmos_sensor_byte(0x0022) << 8) & 0xff00) | (read_cmos_sensor_byte(0x0023) & 0x00ff);
		GbGr_golden_ratio = s5k3p3st_sunny_otp_info.golden_gg = ((read_cmos_sensor_byte(0x0024) << 8) & 0xff00) | (read_cmos_sensor_byte(0x0025) & 0x00ff);
	}
	else {
		...
		LOG_INF("s5k3p3st_sunny_c300_read_eeprom_data info_awb is empty or invalid\n");
		//return false;
	}

	LOG_INF("s5k3p3st_sunny_otp_info module_id = 0x%x,lens_id = 0x%x\n",s5k3p3st_sunny_otp_info.module_id,s5k3p3st_sunny_otp_info.lens_id);
	LOG_INF("s5k3p3st_sunny_otp_info year = %d,month = %d,day = %d\n",  s5k3p3st_sunny_otp_info.year,s5k3p3st_sunny_otp_info.month,s5k3p3st_sunny_otp_info.day);
	printk("s5k3p3st_sunny_otp_info rg_gain = 0x%x,bg_gain = 0x%x,gg_gain = 0x%x\n",s5k3p3st_sunny_otp_info.rg_gain,s5k3p3st_sunny_otp_info.bg_gain,s5k3p3st_sunny_otp_info.gg_gain);
	//lsc data
	uint16_t i, j = 0,check_sum = 0;
	memset(s5k3p3st_sunny_c300_lsc_data,0,1868);
	s5k3p3st_sunny_otp_info.lsc_flag = read_cmos_sensor_byte(0x0027);
	if(s5k3p3st_sunny_otp_info.lsc_flag == 0x01){
		for(i = 0x0028; i <= 0x0773; i++){
			s5k3p3st_sunny_c300_lsc_data[j] =  read_cmos_sensor_byte(i);
			check_sum +=  s5k3p3st_sunny_c300_lsc_data[j];
			j++;
		}
#if 0
		for(i = 0; i <= 1867; i++){
			printk("s5k3p3st_sunny_c300_read_eeprom_data  s5k3p3st_sunny_c300_lsc_data[%d]=%d\n",i,s5k3p3st_sunny_c300_lsc_data[i]);
				}
#endif

	}
	else{
		LOG_INF("s5k3p3st_sunny_otp_info lsc data is empty or invalid\n");

	}

	if(read_cmos_sensor_byte(0x0774) == check_sum % 256){
		printk("s5k3p3st_sunny_otp_info lsc data is suceess\n");
	}

	return true;
}

static void s5k3p3st_sunny_c300_update_wb(void)
{
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			...
			if (*sensor_id ==imgsensor_info.sensor_id) {
				...
				s5k3p3st_sunny_c300_read_eeprom_data();
				return ERROR_NONE;
			}
...
//kin0603
UINT32 S5K3P3ST_SUNNY_C300_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
