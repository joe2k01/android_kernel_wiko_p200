...
#define APS_FUN(fmt, args...)		pr_err(APS_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_128, EPL_ALS_INTT_128};//{EPL_ALS_INTT_1024, EPL_ALS_INTT_1024};
...
static int als_dynamic_intt_low_thr[] = {200,200};//{200, 6000};
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.als.report_type = CMC_BIT_DYN_INT; //CMC_BIT_RAW; //CMC_BIT_DYN_INT; //CMC_BIT_RAW //CMC_BIT_INTR_LEVEL
...
static int epl259x_als_factory_get_raw_data(int32_t *data)
{
    ...
    als_raw = factory_als_data();
    APS_LOG("[%s]: als_raw=%d \r\n", __func__, als_raw);
    //snprintf(*data + len, PAGE_SIZE - len, "%d", als_raw);
#endif
	return 0;
}
...
*/
	...
	if(1 != epl_sensor_I2C_Read(client, DEVREG_CHIP_ID, 1))
        epl_sensor_I2C_Read(client, DEVREG_CHIP_ID, 1);
	printk("%s:  gRawData.raw_bytes[0]=%x\n", __func__, gRawData.raw_bytes[0]);
 ...
 //   if((err = misc_register(&epl_sensor_device)))
    ...
    FULL_PRODUCT_DEVICE_INFO(ID_PSENSOR, "MN26233T");
#else
