...
#define TINNO_PS_DYNAMIC_REAL_CALI
#define TINNO_PS_DYNAMIC_REAL_CALI_TIMER 55
...
#define APS_FUN(f)              printk(KERN_INFO APS_TAG LTR578_DEV_NAME" %s\n", __FUNCTION__)
#define APS_ERR(fmt, args...)   printk(KERN_ERR APS_TAG LTR578_DEV_NAME" %d %s : "fmt, __LINE__, __FUNCTION__, ##args)
#define APS_LOG(fmt, args...)   printk(KERN_NOTICE APS_TAG LTR578_DEV_NAME fmt, ##args)
#define APS_DBG(fmt, args...)   printk(KERN_DEBUG APS_TAG LTR578_DEV_NAME fmt, ##args)
...
#ifdef TINNO_PS_DYNAMIC_REAL_CALI
    struct hrtimer		hrTimer;
    ktime_t			target_ktime;
    struct workqueue_struct	*ps_real_cali_workqueue;
    struct work_struct  ps_real_cali_work;
    u16 		ps_min;
    u16 		ps_max;
#endif
};
...
#ifdef TINNO_PS_DYNAMIC_REAL_CALI
static int ps_real_dynamic_cali_enabled = 0;
#endif
...
#ifdef TINNO_PS_DYNAMIC_REAL_CALI
static void ps_initTimer(struct hrtimer *timer, enum hrtimer_restart (*callback)(struct hrtimer *))
{
	hrtimer_init(timer, CLOCK_MONOTONIC, HRTIMER_MODE_ABS);
	timer->function = callback;
}

static void ps_startTimer(struct hrtimer *timer, int delay_ms, bool first)
{
	struct ltr578_priv *obj = (struct ltr578_priv *)container_of(timer, struct ltr578_priv, hrTimer);
	static int count;
...
static void ps_stopTimer(struct hrtimer *timer)
{
	hrtimer_cancel(timer);
}
#endif

static int psdata_mean = 0;
static int psdata_total = 0;
static int ps_count = 0;
static void ps_real_cali_work_func(struct work_struct *work)
{
    int pdata_delta;
    struct ltr578_priv *obj = ltr578_obj;

    if(ps_real_dynamic_cali_enabled == 0)
        return;
    ...
    if(obj->ps > 0)
    {
        if(0 == obj->ps_min)     //avoid first invalid data
        {
            if(0 == obj->ps_max)
            {
                obj->ps_min = obj->ps;
            ...
            {
                obj->ps_min = (obj->ps_max - obj->ps) > 0 ? obj->ps: obj->ps_max;
            }
        }

        if(obj->ps > obj->ps_max)
            obj->ps_max = obj->ps;
        else if(obj->ps < obj->ps_min)
            obj->ps_min = obj->ps;

        ps_count ++;
        psdata_total += obj->ps;
        psdata_mean = psdata_total/ps_count;
        APS_ERR("PROXIMITY_timer: min data %d, max data %d, current raw data %d, mean raw data %d\n", obj->ps_min,
                    obj->ps_max, obj->ps,psdata_mean);
        if(ps_count > 50 )
        {
            ps_count = 0;
            psdata_mean = 0;
    ...
    pdata_delta = abs(obj->ps_max - obj->ps_min);

    //APS_ERR("PROXIMITY_timer: min data %d, max data %d, current raw data %d\n", obj->ps_min,
    //            obj->ps_max, obj->ps);


    if (pdata_delta > LTR578_PS_DATA_DIFF && obj->ps_min > 0) //avoid first invalid data
    {
        //update new threshold
        atomic_set(&obj->ps_thd_val_high, (obj->ps_min + LTR578_PS_THRESH_NEAR_OFFSET));
        atomic_set(&obj->ps_thd_val_low, (obj->ps_min + LTR578_PS_THRESH_FAR_OFFSET));
// todo--save to NV

        //update threshold registers
        ltr578_ps_set_thres();
        APS_ERR("PROXIMITY_Done: delta > 300\n");
        APS_ERR("PROXIMITY_Done: threshold_near %d, threshold_far %d\n", atomic_read(&obj->ps_thd_val_high),
                        atomic_read(&obj->ps_thd_val_low));
        ps_real_dynamic_cali_enabled = 0;
    }
    else
    {
        APS_ERR("PROXIMITY_Go: delta < 300\n");
        APS_ERR("PROXIMITY_Go: threshold_near %d, threshold_far %d\n", atomic_read(&obj->ps_thd_val_high),
                        atomic_read(&obj->ps_thd_val_low));
        ps_startTimer(&obj->hrTimer, TINNO_PS_DYNAMIC_REAL_CALI_TIMER, false);
    }
	return;
}


enum hrtimer_restart ps_timer_handler(struct hrtimer *timer)
{
	struct ltr578_priv *obj = (struct ltr578_priv *)container_of(timer, struct ltr578_priv, hrTimer);

	queue_work(obj->ps_real_cali_workqueue, &obj->ps_real_cali_work);
...
static int ltr578_ps_enable(struct i2c_client *client, int enable)
{
	...
	if (enable != 0)
    {
        ...
        ltr578_obj->ps_min = 0;
        ltr578_obj->ps_max = 0;
        ps_real_dynamic_cali_enabled = 1;
        ps_startTimer(&ltr578_obj->hrTimer, TINNO_PS_DYNAMIC_REAL_CALI_TIMER, true );
        ps_count = 0;
        psdata_mean = 0;
        psdata_total = 0;
#endif
...
#ifdef TINNO_PS_DYNAMIC_REAL_CALI
        ps_stopTimer(&ltr578_obj->hrTimer);
        ps_real_dynamic_cali_enabled = 0;
        ps_count = 0;
        psdata_mean = 0;
        psdata_total = 0;
...
static int ltr578_als_read(struct i2c_client *client, u16* data)
{
    ...
    if(ratio > 100)
    {
        APS_DBG("ltr578_als_read:ratio*10:%d > 100, A or HZ light\n",ratio);
        luxdata_int = alsval * 67 / 100;        //formula: ALS counts * 0.8/gain/int , int=1
    }
    else if(ratio > 40)     /* D65 light*/
    {
        APS_DBG("ltr578_als_read:40 < ratio*10:%d < 100, D65 light\n",ratio);
        luxdata_int = alsval * 78 / 100;
    }
    else if (ratio > 11)    /* U30 light */
    {
        APS_DBG("ltr578_als_read: 11 < ratio*10:%d <40, u30 light\n",ratio);
        luxdata_int = alsval *39/ 10;
    }else if( ratio > 9)    /* TL84 light */
    {
        APS_DBG("ltr578_als_read: 9 < ratio*10:%d <11, TL84 light\n",ratio);
        luxdata_int = alsval *26/10;
    }else                   /* CWF light */
    {
        APS_DBG("ltr578_als_read: ratio*10:%d <9, CWF light\n",ratio);
        luxdata_int = alsval *16/ 10;
    }

#else
	if (ALS_USE_CLEAR_DATA == 1)
	{
		// ALS_Lux = ALS_DATA * 0.8 * WINFAC1 * (1 \96 WINFAC2 * CLEAR_DATA / ALS_DATA) / ALS_GAIN / ALS_INT
#if 0
...
static int ltr578_get_ps_value(struct ltr578_priv *obj, u16 ps)
{
    ...
    APS_ERR("ltr578_get_ps_value MAIN_STATUS Register: %x\n", buffer);
...
#ifdef TINNO_PS_DYNAMIC_REAL_CALI
    INIT_WORK(&obj->ps_real_cali_work, ps_real_cali_work_func);
    obj->ps_real_cali_workqueue = NULL;
    obj->ps_real_cali_workqueue = create_workqueue("ps_real_cali_work");
    if (!obj->ps_real_cali_workqueue) {
        kfree(obj);
		goto exit_init_failed;
    }
    ps_initTimer(&obj->hrTimer, ps_timer_handler);
#endif
...
#ifdef CONFIG_TINNO_PRODUCT_INFO
#if defined(CONFIG_PROJECT_C200)
    FULL_PRODUCT_DEVICE_INFO(ID_PSENSOR, "LTR579");
#else
    FULL_PRODUCT_DEVICE_INFO(ID_PSENSOR, ltr578_init_info.name);
#endif
#endif
