...
static int icm206xx_sc_WriteDMPImage(void)
{
	...
	res = icm206xx_share_i2c_write_memory(ICM206XX_DMP_CODE_START_LOC,
					...
					&dmpMemory[0]);
	if (res < 0) {
		STEP_C_PR_ERR("memory write error!!\n");
...
static int icm206xx_sc_ReadDMPImage(void)
{
	...
	res = icm206xx_share_i2c_read_memory(ICM206XX_DMP_CODE_START_LOC,
					...
					&dmpVerify[0]);
	if (res < 0) {
		STEP_C_PR_ERR("memory read error!!\n");
		return ICM206XX_ERR_I2C;
	}
...
static int icm206xx_sc_setDMPStartAddress(int addr)
{
	...
	if (res < 0) {
		STEP_C_PR_ERR("set DMP start address by write START_ADDRH register err!\n");
		return ICM206XX_ERR_I2C;
	}
...
static int icm206xx_sc_startDMP(void)
{
	...
	if (res < 0) {
		STEP_C_PR_ERR("start DMP by write USER_CTL register err!\n");
		return ICM206XX_ERR_I2C;
	}
...
static int icm206xx_sc_EnableStepCounter(u8 en)
{
	...
	icm206xx_sc_stopDMP();
		STEP_C_PR_ERR("enable/disable pedometer err!en = %d\n",en);
	if (en == 1)
	...
	if (res < 0) {
		STEP_C_PR_ERR("enable/disable pedometer err!\n");
		return ICM206XX_ERR_I2C;
	}
...
static int icm206xx_sc_init_client(bool enable)
{
	...
	printk("JH icm206xx_sc_init_client");
	...
	printk("JH exit icm206xx_sc_init");
...
//icm206xx TBD
static int icm206xx_floor_c_get_data(uint32_t *value, int *status)
{
...
static int icm206xx_sc_floor_c_set_delay(u64 delay)
{
	return 0;
}

static int icm206xx_sc_step_c_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return 0;
}
...
static int icm206xx_sc_floor_c_flush(void)
{
	return floor_c_flush_report();
}

static int icm206xx_sc_smd_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return 0;
}

static int icm206xx_sc_smd_flush(void)
{
	return smd_flush_report();
}

static int icm206xx_sc_floor_c_enable_nodata(int en)
{
	
		if (1 == en) {
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_FLOOR_COUNTER, true);
		icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_FLOOR_COUNTER, true);
		icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_FLOOR_COUNTER, 5000000, false);
		icm206xx_sc_EnableStepCounter(en);
	} else {
		icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_FLOOR_COUNTER, 0, false);
		icm206xx_sc_EnableStepCounter(en);
		icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_FLOOR_COUNTER, false);
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_FLOOR_COUNTER, false);
	}

	STEP_C_LOG("icm206xx_floor_c_enable_nodata OK!\n");
	return 0;
}
...
static int icm206xx_sc_attr_create(void)
{
	...
	printk("JH icm206xx_sc_attr_create");
	...
	if (res) {
		STEP_C_PR_ERR("icm206xx_a_device misc register failed!\n");
	...
	if (res) {
		STEP_C_PR_ERR("icm206xx_sc create attribute err = %d\n", res);
	...
	ctl.floor_c_set_delay  = icm206xx_sc_floor_c_set_delay;		/* function pointer */
	ctl.step_c_batch = icm206xx_sc_step_c_batch;
	ctl.step_c_flush = icm206xx_sc_step_c_flush;
	ctl.step_d_batch = icm206xx_sc_step_d_batch;
	ctl.step_d_flush = icm206xx_sc_step_d_flush;
	ctl.floor_c_batch = icm206xx_sc_floor_c_batch;
	ctl.floor_c_flush = icm206xx_sc_floor_c_flush;
	ctl.smd_batch = icm206xx_sc_smd_batch;
	ctl.smd_flush = icm206xx_sc_smd_flush;
	ctl.is_report_input_direct = false;
	ctl.is_counter_support_batch = false;
	ctl.is_detector_support_batch = true;
	ctl.is_smd_support_batch = false;
	ctl.is_floor_c_support_batch = false;
	ctl.enable_floor_c = icm206xx_sc_floor_c_enable_nodata;
	...
	data.get_data_floor_c = icm206xx_floor_c_get_data;
	...
	printk("JH exit icm206xx_sc_attr_create");
...
static int icm206xx_sc_attr_remove(void)
{
	...
	if (res)
		STEP_C_PR_ERR("icm206xx_sc_delete_attr fail: %d\n", res);
	else
		res = 0;
...
static int icm206xx_sc_local_init(void)
{
	...
	printk("JH icm206xx_sc_local_init");
...
static int __init icm206xx_sc_init(void)
{
	...
	printk("JH _init icm206xx_sc_init");
