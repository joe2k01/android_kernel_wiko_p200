...
#define MSE_FUN(f)              printk(KERN_DEBUG MSE_TAG" %s\r\n", __FUNCTION__)
#define MSE_ERR(fmt, args...)   printk(KERN_ERR MSE_TAG" %s %d : \r\n"fmt, __FUNCTION__, __LINE__, ##args)
#define MSE_LOG(fmt, args...)   printk(KERN_DEBUG MSE_TAG fmt, ##args)
#else
#define MAGN_TAG
...
static const struct dev_pm_ops afx133_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(afx133_suspend, afx133_resume)
};
#endif
...
//    pr_err("HJDDbgMag, afx133_get_data--afx133_ReadSensorData \n");
    err = afx133_ReadSensorData(xyz);
    if(err)
    {
        pr_err("HJDDbgMag, afx133_get_data: ReadSensorData fail\n");
        return err;
    }
    
    // afx133_coordinate_convert(xyz);
    
    *x = xyz[0] * CONVERT_M;
    *y = xyz[1] * CONVERT_M;
    *z = xyz[2] * CONVERT_M;
    *status = 1;
//    pr_err("HJDDbgMag, x=%d, y=%d, z=%d \n", *x, *y, *z);
...
//    return  afx133_get_data(&data[0], &data[1], &data[2], status);
	...
	err = afx133_get_data(&data[0], &data[1], &data[2], status);
...
//add for TinnoProductInfo 
#ifdef CONFIG_TINNO_PRODUCT_INFO
	FULL_PRODUCT_DEVICE_INFO(ID_MSENSOR, "AF9133"); 
#endif
    return 0;
