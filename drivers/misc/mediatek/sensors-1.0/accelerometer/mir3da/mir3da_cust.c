...
printk("------mir3da_probe--direction= %d-----client->addr = %x\n", obj->hw.direction, client->addr);     
	{
		for(i=0;i<3;i++)
		{
			i2c_smbus_read((PLAT_HANDLE) client, NSA_REG_WHO_AM_I, &chip_id);
			printk("--mir3da read chip_id = %x\n", chip_id);
			if(chip_id == 0x13)break;	
			mdelay(5); 
		}
	}	
	if(chip_id != 0x13)
	...
	if(chip_id != 0x13)goto exit_init_failed;
...
//add for TinnoProductInfo 
#ifdef CONFIG_TINNO_PRODUCT_INFO
	FULL_PRODUCT_DEVICE_INFO(ID_GSENSOR, "DA218-mir3da");    
#endif
    return result;
