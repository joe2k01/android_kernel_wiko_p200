...
static const struct dev_pm_ops stk8baxx_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(stk8baxx_suspend, stk8baxx_resume)
};
#endif
...
static char califile_path[STK8BAXX_BUF_SIZE] = "/bak/stk8baxx-calib.txt";
static char backup_califile_path[STK8BAXX_BUF_SIZE] = "/dev/block/platform/mtk-msdc.0/11230000.msdc0/by-name/proinfo";
...
#define _WRT_LOG_DATA_BUFFER_SIZE    (GSENSOR_CALI_DATA_LEN)
	...
	fd_file = openFile(califile_path, O_RDWR | O_CREAT, 0644);
	if (fd_file == NULL) {
		GSE_LOG("stk8baxx_write_log_data fail to open\n");

		/* fd_file = openFile(backup_califile_path ,O_RDWR,0); */
		fd_file = openFile(backup_califile_path, O_RDWR, 0);

		if (fd_file == NULL) {
			GSE_LOG("%s:fail to open proinfo file: %s\n", __func__, backup_califile_path);
			set_fs(oldfs);
			return (-1);
		}
		GSE_LOG("Open proinfo file successfully: %s\n", backup_califile_path);
...
static int stk8baxx_read_cali_file(struct i2c_client *client)
{
	...
	initKernelEnv();
	fd_file = openFile(califile_path, O_RDONLY, 0);

	if (fd_file == NULL) {
		GSE_LOG("%s:fail to open calibration file: %s\n", __func__, califile_path);
		fd_file = openFile(backup_califile_path, O_RDONLY, 0);
...
static int STK8BAXX_ReadCalibration(struct i2c_client *client, int dat[STK8BAXX_AXES_NUM])
{
	...
	GSE_LOG("%s:before stk8ba50_readoffset\n", __func__);
		...
		dat[obj->cvt.map[STK8BAXX_AXIS_X]] =
		    obj->cvt.sign[STK8BAXX_AXIS_X] * obj->cali_sw[STK8BAXX_AXIS_X];
		dat[obj->cvt.map[STK8BAXX_AXIS_Y]] =
		    obj->cvt.sign[STK8BAXX_AXIS_Y] * obj->cali_sw[STK8BAXX_AXIS_Y];
		dat[obj->cvt.map[STK8BAXX_AXIS_Z]] =
		    obj->cvt.sign[STK8BAXX_AXIS_Z] * obj->cali_sw[STK8BAXX_AXIS_Z];
	#else
...
#ifdef SW_CALIBRATION
	obj->cali_sw[STK8BAXX_AXIS_X] = obj->cvt.sign[STK8BAXX_AXIS_X]*(cali[obj->cvt.map[STK8BAXX_AXIS_X]]);
	obj->cali_sw[STK8BAXX_AXIS_Y] = obj->cvt.sign[STK8BAXX_AXIS_Y]*(cali[obj->cvt.map[STK8BAXX_AXIS_Y]]);
	obj->cali_sw[STK8BAXX_AXIS_Z] = obj->cvt.sign[STK8BAXX_AXIS_Z]*(cali[obj->cvt.map[STK8BAXX_AXIS_Z]]);	
#else
...
printk("\n[cyamon]%s:\n",__func__);
	...
	if (err) {
		GSE_ERR("stk8baxx_accel_WriteCalibration failed!\n");
		return -1;
	}

printk("\n[cyamon]%s:cali data [%d][%d][%d]\n",__func__, cali[STK8BAXX_AXIS_X], cali[STK8BAXX_AXIS_Y], cali[STK8BAXX_AXIS_Z]);	
        return 0;	
}
...
static int stk8baxx_factory_clear_cali(void)
{
        ...
        if (err) {
                GSE_ERR("stk8baxx_accel_factory_clear_cali failed!\n");
                return -1;
        }
        return 0;
}
...
static int stk8baxx_factory_get_raw_data(int32_t data[3])
{
        ...
        ACC_LOG("support stk8baxx_accel_factory_get_raw_data!\n");
        return 0;
}
...
static int stk8baxx_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
        ...
        if (err) {
                GSE_ERR("%s enable accel sensor failed!\n", __func__);
                return -1;
        }
        err = stk8baxx_batch(0, sample_periods_ms * 1000000, 0);
        if (err) {
                GSE_ERR("%s enable accel set batch failed!\n", __func__);
                return -1;
        }
        return 0;
}
...
#ifdef CONFIG_TINNO_PRODUCT_INFO
        FULL_PRODUCT_DEVICE_INFO(ID_GSENSOR, "STK8BA50-R");
#endif
