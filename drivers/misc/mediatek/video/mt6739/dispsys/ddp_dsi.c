...
#ifdef CONFIG_LCM_LONG_PKG
static unsigned int DSI_esd_check_times(LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
	static unsigned int esd_check_times=0;
	...
	printk("HDJDbgESD, DSI, 1.ESD times %d\n", esd_check_times);
	return esd_check_times;
}
#endif
...
int ddp_dsi_build_cmdq(enum DISP_MODULE_ENUM module, void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	int j=0; 
	struct DSI_T0_INS t1; 
	struct DSI_RX_DATA_REG read_data1, read_data2, read_data3;
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (packet_type == 0x1A || packet_type == 0x1C) {
				recv_data_cnt = read_data0.byte1 + read_data0.byte2 * 16;
				printk("DSI [ESD]packet_type=0x%x,recv_data_cnt = %d\n", packet_type, recv_data_cnt);
				if(recv_data_cnt > RT_MAX_NUM)
				{
					printk("DSI [ESD] read long packet data exceeds 10 bytes \n");
				...
				for (j = 0; j < recv_data_cnt; j++) {
					printk("DSI [ESD]buffer[%d]=0x%x\n", j, buffer[j]);
					if (buffer[j] != dsi_params->lcm_esd_check_table[i].para_list[j]) {
						result= 1;
						printk("DSI [ESD]CMP i %d return value 0x%x,para_list[%d]=0x%x\n", i,
			...
			packet_type == 0x22) {
				...
				printk("DSI [ESD]read return type is 0x02\n");
				result = 1;
			} else {
				printk("DSI [ESD] read return type is non-recognite, type = 0x%x\n", packet_type);
				result = 1;
			}
