...
extern char Market_Area[16]; //<Line>india Panasonic current =100ma;xiongdajun
/* define usage count */
...
static int aw3643_set_level_ch1(int level)
{
 ...
 pr_err("Market_Area=%s, \n", Market_Area);
	...
	if(0==level){
		...
		//<Begin>india Panasonic current =100ma;xiongdajun
		if(!strcmp(Market_Area, "pan_in_om"))
		{
			val =  0x21;	//100mA//aw3643_torch_level[level];
		}
		else
		val =  0x32;//150mA//aw3643_torch_level[level];
		//<END>india Panasonic current =100ma;xiongdajun
		printk("xiongdajun = %d\n", val);
