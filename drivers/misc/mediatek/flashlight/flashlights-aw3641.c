...
#define AW3641_NAME "flashlights_aw3641"
...
static int aw3641_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_TIME_OUT_TIME_MS:
		printk("aw3641 FLASH_IOC_SET_TIME_OUT_TIME_MS(%d): %d\n",
				channel, (int)fl_arg->arg);
		aw3641_timeout_ms = fl_arg->arg;
		break;

	case FLASH_IOC_SET_DUTY:
		printk("aw3641_FLASH_IOC_SET_DUTY(%d): %d\n",
				channel, (int)fl_arg->arg);
		aw3641_set_level(fl_arg->arg);
		break;

	case FLASH_IOC_SET_ONOFF:
		printk("aw3641 FLASH_IOC_SET_ONOFF(%d): %d\n",
				channel, (int)fl_arg->arg);
		if (fl_arg->arg == 1) {
