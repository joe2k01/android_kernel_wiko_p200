...
// TINNO BEGIN
// huaidong.tan , CDAAOB-435 , DATE20171214 , recognization for CTIA & OMTP
/* add for new feature PIN recognition */
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
static int s_cable_pin_recognition;
static int s_show_icon_delay;
#endif
//TINNO END
...
static inline void check_cable_type(void)
{
	...
	case PLUG_OUT:
		...
		if (current_status == ACCDET_STATE_ABC_00) {
			...
			pmic_pwrap_write(ACCDET_CON04, REGISTER_VALUE(accdet_cust_setting->pwm_width));//pwm_thresh?
	        ...
	        /*A=bit2; B=bit1;C=bit0*/
	        current_status = ((reg_val>>ACCDET_STATE_MEM_BIT_OFFSET)&ACCDET_STATE_ABC_MASK);
			if (current_status == ACCDET_STATE_ABC_00 && s_show_icon_delay != 0) {
				...
				if (pin_adc_value < 180 && pin_adc_value > 90) {	/* 90mv illegal headset */
					...
					if (s_eint_accdet_sync_flag == 1) {
						...
						s_cable_pin_recognition = 1;
						ACCDET_DEBUG("[Accdet] s_cable_pin_recognition = %d\n",
							     s_cable_pin_recognition);
					} else {
...
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
	if (s_cable_pin_recognition == 1) {
		s_cable_pin_recognition = 0;
		accdet_FSA8049_disable();//defined in CONFIG_ACCDET_PIN_SWAP
...
#ifdef CONFIG_ACCDET_PIN_RECOGNIZATION
	accdet_set_debounce(accdet_state000, accdet_cust_setting->debounce0);
	accdet_set_debounce(accdet_state010, 0xFFFF);	/* 2.0s */
	accdet_set_debounce(accdet_state110, accdet_cust_setting->debounce3);
	accdet_set_debounce(accdet_auxadc, accdet_cust_setting->debounce4);
#else
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	// TINNO BEGIN
	// huaidong.tan , CDAAOB-435 , DATE20171214 , add for FTM test, NEVER REMOVE!!!
