...
{
	// TINNO BEGIN
	// huaidong.tan , CCAAGO-186 , DATE20180122 , configure external audio amplifier mode to 6 (0.7W)
	int extamp_mode = 3; /* default mode is 3 */
	//Xucs add start for PA mode select with perso start
	#ifdef CONFIG_TINNO_PERSO
       if(g_PaMode)
	    extamp_mode = g_PaMode;
	printk("extamp_mode is %d\n",extamp_mode);
	#endif
	//Xucs add start for PA mode select with perso end
#if defined(CONFIG_PROJECT_C200)
	extamp_mode = 4; /* set AW8737S mode to 4 is about 0.7W, according to the speaker selecting on C200*/
#endif
	// TINNO END
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
		...
		AudDrv_GPIO_EXTAMP_Select(false, extamp_mode);
		/*udelay(1000); */
		usleep_range(1 * 1000, 2 * 1000);
		AudDrv_GPIO_EXTAMP_Select(true, extamp_mode);
